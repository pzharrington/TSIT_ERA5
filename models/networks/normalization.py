...
class FADE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, ppad=False):
        ...
        pw = ks // 2
        convpad = 0 if ppad else pw
        self.pad = PeriodicPad2d(pw) if ppad else nn.Identity()
        self.mlp_gamma = nn.Conv2d(label_nc, norm_nc, kernel_size=ks, padding=convpad)
        self.mlp_beta = nn.Conv2d(label_nc, norm_nc, kernel_size=ks, padding=convpad)
    ...
    def forward(self, x, feat):
        ...
        pfeat = self.pad(feat)
        gamma = self.mlp_gamma(pfeat)
        beta = self.mlp_beta(pfeat)
